					...
					struct kobj_attribute *attr, char *buf)
{
	...
	if (get_kernel_flag() & KERNEL_FLAG_ENABLE_SSR_MODEM)
	{
		...
		ret = smsm_change_state_ssr(SMSM_APPS_STATE, 0, SMSM_APPS_SEND_MODEM_FATAL, KERNEL_FLAG_ENABLE_SSR_MODEM);

		if(ret == 0)
			pr_info("%s: set smsm state SMSM_APPS_SEND_MODEM_FATAL success\n", __func__);
		else
		{
			pr_info("%s: set smsm state SMSM_APPS_SEND_MODEM_FATAL faild => ret = %d\n", __func__, ret);
					...
					struct kobj_attribute *attr, char *buf)
{
	...
	if((get_radio_flag() & 0x8) && !(get_kernel_flag() & KERNEL_FLAG_ENABLE_SSR_MODEM))
	{
		...
		arch_reset(RESTART_MODE_LEGECY, "oem-95");
	}
	else
	{
