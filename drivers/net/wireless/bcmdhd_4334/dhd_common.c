...
int dhd_set_pktfilter(dhd_pub_t * dhd, int add, int id, int offset, char *mask, char *pattern)
{
	...
	printf("%s: Enter set packet filter, %s %d\n", __FUNCTION__, (add)?"add":"del", id);
	...
	if (LowPowerMode == 1) {
		if (add == 1 && pkt_id == 105) {
			...
			bcm_mkiovar("pm_ignore_bcmc", (char *)&ignore_bcmc,
				4, buf, sizeof(buf));
			printf("set pm_ignore_bcmc: %d",ignore_bcmc);
			dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, buf, sizeof(buf), TRUE, 0);
			/* Disable keep alive packet*/
			dhd_set_keepalive(0);
		} else if (add == 0 && pkt_id == 105) {
			printf("MCAST packet filter, hasDLNA is false\n");
			hasDLNA = false;
			if (is_screen_off && !allowMulticast) {
