...
void wl_iw_slient_restart(struct work_struct *work);
DECLARE_DELAYED_WORK(slient_restart_sta, wl_iw_slient_restart);
...
//BRCM APSTA START
#ifdef APSTA_CONCURRENT 
extern int slient_restart;

//int slient_cnt = 0;
extern int wldev_set_apsta(struct net_device *dev, bool enable);
extern int wl_android_wifi_off(struct net_device *dev);
extern int wl_android_wifi_on(struct net_device *dev);

static char *pkt_filter_disabled_buf[WLC_IOCTL_MAXLEN];
static int pkt_filter_disabled_count = 0;
static char *pkt_filter_enabled_buf[WLC_IOCTL_MAXLEN];
static int pkt_filter_enabled_count = 0;

void
wl_iw_backup_pkt_filter(struct net_device *dev)
{
	int error; 
	uint32 enable;
	...
	enable = htod32(0);
	error = dev_iw_iovar_getbuf(dev, "pkt_filter_list", &enable, sizeof(enable), pkt_filter_disabled_buf, sizeof(pkt_filter_disabled_buf));
	list = (wl_pkt_filter_list_t *)pkt_filter_disabled_buf;
	pkt_filter_disabled_count = (error)?0:list->num; 
	printf("%s: pkt_filter_disabled_count=%d\n", __FUNCTION__, pkt_filter_disabled_count);	

	enable = htod32(1);
	error = dev_iw_iovar_getbuf(dev, "pkt_filter_list", &enable, sizeof(enable), pkt_filter_enabled_buf, sizeof(pkt_filter_enabled_buf));
	list = (wl_pkt_filter_list_t *)pkt_filter_enabled_buf;
	pkt_filter_enabled_count = (error)?0:list->num; 
	printf("%s: pkt_filter_enabled_count=%d\n", __FUNCTION__, pkt_filter_enabled_count);	
}

void
wl_iw_restore_pkt_filter(struct net_device *dev, int enabled)
{
	wl_pkt_filter_list_t *list;
	wl_pkt_filter_t	*filterp;
	char * pkt_filterp = NULL;
	int filter_len = 0;
	int i = 0;
	char buf[256];
	int error = 0;
	int pkt_filter_count;

	pkt_filter_count = (enabled)?pkt_filter_enabled_count:pkt_filter_disabled_count;
        list = (wl_pkt_filter_list_t *)((enabled)?pkt_filter_enabled_buf:pkt_filter_disabled_buf);

	if ( pkt_filter_count ) {
		printf("%s: restore %s list\n", __FUNCTION__, (enabled)?"enabled":"disabled");
		filterp = list->filter;
			...
			int pkt_id = dtoh32(filterp->id);
        		...
        		buf[str_len] = '\0';
       	 		buf_len = str_len + 1;
			pkt_filterp = buf + str_len + 1;

                	filter_len = WL_PKT_FILTER_FIXED_LEN;
                	filter_len += WL_PKT_FILTER_PATTERN_FIXED_LEN + 2 * filterp->u.pattern.size_bytes;
			memcpy( pkt_filterp, filterp, filter_len);
			buf_len += filter_len;

       			if ((error = dev_wlc_ioctl(dev, WLC_SET_VAR, buf, buf_len)) == 0) {
        			enable_parm.id = htod32(pkt_id);
        			enable_parm.enable = htod32(enabled);
        			buf_len = bcm_mkiovar("pkt_filter_enable", (char *)&enable_parm,
                			sizeof(wl_pkt_filter_enable_t), buf, sizeof(buf));
        			dev_wlc_ioctl(dev, WLC_SET_VAR, buf, buf_len);
        		}
...
void
wl_iw_slient_restart(struct work_struct *work)
{
	wlc_ssid_t ssid;
        int error;
	int wsec, wpa_auth, infra;
	scb_val_t scbval;
	int ssid_len;

	printf("%s: start slient restart\n", __FUNCTION__);
	slient_restart = 2;
	
        error = dev_wlc_ioctl(priv_dev, WLC_GET_SSID, &ssid, sizeof(ssid));
        ssid_len = dtoh32(ssid.SSID_len);
	if (!error && ssid_len){

		wl_iw_backup_pkt_filter(priv_dev);

		if ((error = dev_wlc_intvar_get(priv_dev, "wsec", &wsec)))
			return;

		if ((error = dev_wlc_intvar_get(priv_dev, "wpa_auth", &wpa_auth)))
			return;

		if ((error = dev_wlc_ioctl(priv_dev, WLC_GET_INFRA, &infra, sizeof(infra))))
			return;
      
        	bzero(&scbval, sizeof(scb_val_t));
		scbval.val = 8;
		printf("%s: do disassoc\n", __FUNCTION__);
                if ((error = dev_wlc_ioctl(priv_dev, WLC_DISASSOC, &scbval, sizeof(scb_val_t)))) {
                	WL_ERROR(("%s: WLC_DISASSOC failed (%d).\n", __FUNCTION__, error));
               	}

		printf("wsec = %d, wpa_auth = %d, infra = %d\n", wsec, wpa_auth, infra);
		wl_android_wifi_off(priv_dev);
		bcm_mdelay(500);
		wl_android_wifi_on(priv_dev);

		dev_wlc_intvar_set(priv_dev, "wsec", wsec);
		dev_wlc_intvar_set(priv_dev, "wpa_auth", wpa_auth);
		dev_wlc_ioctl(priv_dev, WLC_SET_INFRA, &infra, sizeof(infra));

		printf("%s: set ssid %s\n", __FUNCTION__, ssid.SSID);
		if ((error = dev_wlc_ioctl(priv_dev, WLC_SET_SSID, &ssid, sizeof(ssid)))) {
               		WL_ERROR(("%s: WLC_SET_SSID failed (%d). Stop slient restart.\n", __FUNCTION__, error));
			slient_restart = 0;
		}

		wl_iw_restore_pkt_filter(priv_dev, 0);
		wl_iw_restore_pkt_filter(priv_dev, 1);
	}
}
...
int wl_softap_stop(struct net_device *dev)
{
	...
	int err;
        ...
        if ((ap_cfg_running == TRUE)) {
                ...
                if ((err = dev_wlc_intvar_get(priv_dev, "wpa_auth", &val))){
                        WL_ERR(("could not get wpa_auth (%d)\n", err));
		}

		if (!err && (val & (WPA_AUTH_PSK | WPA2_AUTH_PSK))) {
			/* slient restart */
			printf("schedule slient restart!\n"); 
			schedule_delayed_work(&slient_restart_sta, 20*HZ);
		}
        } else
...
void wl_iw_detach(void)
{
	...
	cancel_delayed_work_sync(&slient_restart_sta);
#endif
}
