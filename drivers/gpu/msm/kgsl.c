						...
						void *data)
{
	...
	struct kgsl_context *context;
	int result;
	static unsigned long last_waittimestamp_jiffies = 0;
	static int last_pid = 0;
	int current_pid = 0;
	unsigned long wait_time_jiff = 0;

	if (dev_priv != NULL && dev_priv->process_priv != NULL)
		current_pid = dev_priv->process_priv->pid;

	wait_time_jiff = last_waittimestamp_jiffies + msecs_to_jiffies(500);
	last_waittimestamp_jiffies = jiffies;
	context = kgsl_find_context(dev_priv, param->context_id);

	/*
	 * Lots of invalid context_id error in the same process that range
	 * over a short amount of time are likely to cause watchdog or dogbark
	 * issue, so it tries to throttle the number of messages being printed.
	 */
	if (context == NULL) {
		if ((last_pid != current_pid) || (time_after(jiffies, wait_time_jiff))) {

			KGSL_DRV_ERR(dev_priv->device, "invalid context_id %d current pid=%d\n", param->context_id, current_pid);
