			...
			unsigned int cmd)
{
   ...
   if ( (unsigned long)handle->buffer->heap->ops->cache_op < 0xc0000000 )
       pr_err("gpuion: %s: invalid cache_op %p, id %d)\n", __func__, handle->buffer->heap->ops->cache_op, handle->buffer->heap->id);
